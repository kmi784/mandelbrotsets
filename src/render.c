 ...
 * @return `RGB` object of corresponding color 
 */
static RGB inferno_colormap(double t) {
    RGB c0 = {0, 0, 4};
    RGB c1 = {31, 12, 72};
    RGB c2 = {85, 15, 109};
    RGB c3 = {187, 55, 84};
    ...
    if(t < 0.25) {
        return linear_interpolation(t / 0.25, c0, c1);
    } else if(t < 0.5) {
        return linear_interpolation((t - 0.25) / 0.25, c1, c2);
    } else if(t < 0.75) {
        return linear_interpolation((t - 0.5) / 0.25, c2, c3);
    } else {
        return linear_interpolation((t - 0.75) / 0.25, c3, c4);
    }
}
...
static RGB colormap(ColorMap cmap, double pixel_value) {
    ...
    switch(cmap) {
        case COLORMAP_BLUE:
            t = fmod(pixel_value * 0.008, 1.0);
            return blue_colormap(t);

        case COLORMAP_DARKSOULS:
            t = fmod(pixel_value * 0.005, 1.0);
            return darksouls_colormap(t);

        case COLORMAP_GRAY:
            t = fmod(pixel_value * 0.006, 1.0);
    ...
    for(unsigned int i = 0; i < image->height; ++i) {
        for(unsigned int j = 0; j < image->width; ++j) {
            ...
            if(pixel_value < 0.0) {
                ...
                if(
                    assign_image_pixel(
                        image, i, j, colormap(cmap, pixel_value)
                    )
                ) {return 1;}
            }
        }
